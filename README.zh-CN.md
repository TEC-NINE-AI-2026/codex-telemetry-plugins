...
这是一个可通过 Git 分发的 Codex 插件 Marketplace，其中包含 **Codex 性能面板**：一个在本机运行的只读面板，用于查看任务延迟、阶段耗时、Token 用量、上下文占用和订阅限额快照。
...
- 汇总 Token、上下文窗口、上下文压缩和可用的订阅限额快照。
- 按项目、任务、状态和时间范围筛选。
...
插件读取本机 Codex 会话元数据，并在当前用户的应用数据目录中写入派生 SQLite 数据库。面板会保存并展示截断后的用户消息和最终回复摘录；设计上不保存认证数据、推理正文、工具命令、工具参数或工具输出，也不会上传分析数据。安装前建议阅读 [隐私说明](PRIVACY.md) 和 [安全说明](SECURITY.md)。
